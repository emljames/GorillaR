---
title: "Tutorial Part 1: From Gorilla to Tidy Data in 5 Easy Steps"
author: "Emma James"
date: "April 2020"
output:
  html_document:
...
*Please note that this document is a first draft - please email comments and feedback to [emma.james@york.ac.uk](emma.james@york.ac.uk).* 
...
```{r}
raw_data <- read.csv("./Story materials/data_exp_4424-v9_task-y5i7.csv")
```
...
write.csv(participant_data, "participant_averages.csv", row.names = FALSE)
```
...
```{r}
processed_data <- read.csv("./Story materials/data_exp_4424-v9_task-y5i7.csv") %>% 
  filter(Zone.Type == "response_button_text") %>%
  select(Participant.Private.ID, ANSWER, Correct, Reaction.Time) %>% 
  set_names(c("ID", "item", "acc", "RT")) %>% 
  group_by(ID) %>% 
